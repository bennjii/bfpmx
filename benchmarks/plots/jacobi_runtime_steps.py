...
grouped = df.groupby(['format','stress_function', 'steps', 'label'])['runtime (ms)'].mean().reset_index()
#print(grouped)
...
ax = sns.lineplot(data=grouped, x="steps", y="runtime (ms)", hue="label", marker="o")
...
plt.legend().remove() 
label_offsets = {
    "Jacobi2DArray": {"x_offset": 0, "y_offset": 2},
    "Jacobi2DAlwaysFastMarshal": {"x_offset": -20, "y_offset": 0.1},
    "Jacobi2DSpreadBlockEach": {"x_offset": -20, "y_offset": -0.35}, 
    "Jacobi2DNaiveBlock":{"x_offset": -20, "y_offset": -10},  
    "Jacobi2DSpreadBlockOnce":{"x_offset": -20, "y_offset": -2},
}
...
plt.ylabel("Runtime (ms)",fontfamily='Calibri', fontsize=14)  
plt.tight_layout()
