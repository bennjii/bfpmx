...
constexpr std::array<u32,9> StepsArray = {2,5,10,25,50,100,250,1000,10000};
...
template <typename Float, typename Dimensions, template <typename> class ArithmeticPolicy>
using TestingBlockT = Block<TestingScalar, Dimensions, Float,
                            ArithmeticPolicy, SharedExponentQuantization>;

template <typename Float, typename Dimensions>
using TestingBlock = TestingBlockT<Float, Dimensions, CPUArithmetic>;

template <typename Float, typename Dimensions>
using TestingBlockNoMarshal =
    TestingBlockT<Float, Dimensions, CPUArithmeticFastMarshalling>;
...
static void Jacobi2DNaiveBlock(const int steps,
                               TestingBlock<Float, BlockDims<N, N>> &A,
                               TestingBlock<Float, BlockDims<N, N>> &B) {
  profiler::func();
...
static void Jacobi2DSpreadBlockEach(const int steps,
                                    TestingBlock<Float,BlockDims<N, N>> &A_block,
                                    TestingBlock<Float,BlockDims<N, N>> &B_block) {
  profiler::func();
  ...
  for (u32 t = 0; t < steps; t++) {
    ...
    B_block = TestingBlock<Float, Dimensions>(b_new_values);
    ...
    A_block = TestingBlock<Float, Dimensions>(a_new_values);
  }
}
...
static void Jacobi2DSpreadBlockOnce(const int steps,
  ...
  A_block = TestingBlock<Float,BlockDims<N, N>>(a_spread);
  B_block = TestingBlock<Float,BlockDims<N, N>>(b_spread);
}
...
Iteration Test(u32 Steps) {
  ...
  using Block = TestingBlock<Float,Size>;
  ...
  Jacobi2DNaiveBlock<Float,N>(Steps, blockA_naive, blockB_naive);

  Block blockA_spread_each(aLinear_base), blockB_spread_each(bLinear_base);
  Jacobi2DSpreadBlockEach<Float,N>(Steps, blockA_spread_each, blockB_spread_each);

  Block blockA_spread_once(aLinear_base), blockB_spread_once(bLinear_base);
  Jacobi2DSpreadBlockOnce<Float,N>(Steps, blockA_spread_once, blockB_spread_once);
...
template<typename... Floats, size_t...Ns>
void runAll(TypeList<Floats...>){
  CsvWriter writer;
  (runForFloat<Floats>(writer), ...);
  writer.dump("../benchmarks/jacobi2dALLnew.csv");
...
void runForFloat(CsvWriter& writer) {
  runForSizes<Float>(writer, SizeList<2,4,8,12,16,20,24,28,32>{});
}

template <typename Float, size_t... Ns>
void runForSizes(CsvWriter &writer, SizeList<Ns...>) {
    (runForSize<Float, Ns>(writer), ...);
}

template <typename Float, size_t N>
void runForSize(CsvWriter &writer) {
  for (u32 Steps: StepsArray){
    ...
    for (int i = 0; i < Iterations; i++) {
      auto [percentage, absolute] = Test<Float, N>(Steps);
