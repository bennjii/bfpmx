 ...
 *
 */
template <u16 Exponent, u16 Significand, u16 Sign> class FloatRepr {
public:
  using PackedForm =
  ...
  static constexpr f64 Next(const f64 value) {
    ...
    auto [sign, exp, frac] =
        Pack(value); // you already have this info in Marshal
    if (sign == 0) { // positive
  ...
  [[nodiscard]] static constexpr std::array<u8, SizeBytes()>
  Marshal(const f64 value) {
    auto [sign, exp, frac] = Pack(value);

    // pack into bits
    const u64 encoded = (sign << (ExponentBits() + SignificandBits())) |
                        (exp << SignificandBits()) |
