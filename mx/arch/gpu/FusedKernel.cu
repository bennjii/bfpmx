...
template <typename FloatD, typename BlockViewT>
__device__ __forceinline__ double PerformArithmetic(double l_val, double r_val, ArithmeticOp op) {
    switch (op) {
...
__global__ void FusedBlockArithmeticKernel(
    ...
    constexpr int MAX_BLOCKS_PER_TB = 32;
    __shared__ double shared_l_scalars[MAX_BLOCKS_PER_TB];
    __shared__ double shared_r_scalars[MAX_BLOCKS_PER_TB];
    ...
    if (local_block < blocks_per_thread_block) {
        ...
        if (block_idx < num_blocks) {
            ...
            if (local_elem_idx == 0) {
                ...
                shared_l_scalars[local_block] = ComputeScalar<FloatD, BlockViewT>(l_scalar_bits);
                shared_r_scalars[local_block] = ComputeScalar<FloatD, BlockViewT>(r_scalar_bits);
            }
        }
    }
    ...
    if (local_block < blocks_per_thread_block) {
        ...
        if (block_idx < num_blocks && local_elem_idx < d_lhs_blocks[block_idx].num_elems) {
            ...
            const double l_scalar = shared_l_scalars[local_block];
            const double r_scalar = shared_r_scalars[local_block];
            ...
            double result = PerformArithmetic<FloatD, BlockViewT>(l_val, r_val, op);
...
void LaunchFusedBlockArithmeticKernel(
    ...
    FusedBlockArithmeticKernel<FloatTypeD, BlockViewT>
        <<<numBlocks, threadsPerBlock>>>(d_lhs_blocks, d_rhs_blocks, d_out_blocks, 
                                         num_blocks, elems_per_block, blocks_per_thread_block, op);
    CUDA_CHECK_KERNEL();
}
...
template
void LaunchFusedBlockArithmeticKernel<BV431>(
    const BV431*, const BV431*, BV431*, uint32_t, uint32_t, ArithmeticOp);

template
void LaunchFusedBlockArithmeticKernel<BV521>(
    const BV521*, const BV521*, BV521*, uint32_t, uint32_t, ArithmeticOp);
