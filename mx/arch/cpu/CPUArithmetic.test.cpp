...
using TestingBlock = Block<4, TestingDimensions, TestingFloat, CPUArithmetic,SharedExponentQuantization>;


TEST_CASE("GEMV") {

    SECTION("2x3 matrix-vec multiplication") {
        using Matrix2x3 = Block<4, BlockDims<2, 3>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        using Vector3 = Block<4, BlockDims<3>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        using Vector2 = Block<4, BlockDims<2>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        ...
        auto result = CPUArithmetic<Vector2>::template Gemv<Matrix2x3, Vector3, Vector2>(matrix, vector);
    ...
    SECTION("3x2 matrix-vec multiplication") {
        using Matrix3x2 = Block<4, BlockDims<3, 2>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        using Vector2 = Block<4, BlockDims<2>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        using Vector3 = Block<4, BlockDims<3>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;

        std::array<f64, Matrix3x2::NumElems> matrix_data = {
            4.0, 2.0,
            8.0, 4.0,
            2.0, 1.0
        };
        ...
        auto result = CPUArithmetic<Vector3>::template Gemv<Matrix3x2, Vector2, Vector3>(matrix, vector);
...
TEST_CASE("GEMM") {
        ...
        using Matrix3x4 = Block<4, BlockDims<3, 4>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        using Matrix4x2 = Block<4, BlockDims<4, 2>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;
        using Matrix3x2 = Block<4, BlockDims<3, 2>, TestingFloat, CPUArithmetic, SharedExponentQuantization>;

        std::array<f64, 12> a_data = {
            1.0, 2.0, 4.0, 8.0,
            2.0, 4.0, 8.0, 1.0,
            4.0, 8.0, 1.0, 2.0
        };
        
        std::array<f64, 8> b_data = {
            2.0, 1.0,
            1.0, 2.0,
            2.0, 1.0,
            1.0, 2.0
        ...
        auto result = CPUArithmetic<Matrix3x2>::template Gemm<Matrix3x4, Matrix4x2, Matrix3x2>(A, B);

        //Correct result: 20 25 25 20 20 25
        //Actual result: 20 24 24 20 20 24 --[!]--> 25 cannot be represented in fp8::E4M3 so it gets rounded down
